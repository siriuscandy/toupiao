define([ 'jquery', 'knockout', 'text!pages/productRelease/add/add.html','dialogmin','ajaxCom'
 ],
    function($, ko, template,dialogmin,ajaxCom) {
        ...
        viewModel.downloadPdf = function(){
             var url = "http://iuap-tenat-market.oss-cn-beijing.aliyuncs.com/download/%E4%BA%91%E5%B8%82%E5%9C%BA%E6%9C%8D%E5%8A%A1%E5%BC%80%E9%80%9A%E6%8E%A5%E5%8F%A3%E6%96%87%E6%A1%A3.pdf"
             window.open(url)
         }
        viewModel.submitMo = function() {
            var apilinkPKData = "";
            //if(viewModel.categoryId=='ebe649e0-5bcd-8638-3242-e17e773490c2'){
                    apilinkPKData = $("#APIdata").val()
            //}
            var neData = {
            ...
            if(neData.productName==undefined||neData.productName==""){
                dialogmin('请填写商品名称~');
                return false
            }if(neData.openMode!=3 && neData.interfaceAddress==""){
                dialogmin('请填写生产系统接口地址~');
                return false
            }
            if(neData.openMode==1 && neData.interfaceAddress!=""){
                if(neData.interfaceAddress.indexOf('http')<0){
                    dialogmin('请填写正确的生产系统接口地址~');
                    return false
                }
            }
            if(neData.openMode==3 ){
                neData.interfaceAddress = '';
            }
        ...
        viewModel.getInfo = function(){
            if(viewModel.productId != ""){
                $.ajax({
                    type : "get",
                    url : $ctx + productUrl+viewModel.productId,
                    data : {isAjax:1},
                    headers: {
                    ...
                    success : function(res) {
                        ...
                        if (res.status==1) {
                            //res.data.productName = decodeURIComponent(res.data.productName)
                            ...
                            if (res.data.openMode == 3) {
                                ...
                                if(res.data.apilinkPK!='' && res.data.apilinkPK!=undefined){
                                    $(".app_com_list_bo_2").show();
                                    $(".select_this").val('1').attr("disabled",'disabled').addClass("gred");
                                    $(".select_this").next().hide();
                                }
                                $(".app_com_list_bo_2").show();
                            }
            ...
            ajaxCom.Loadajax('get',APIdataUrl,{isAjax:1},function(res){//请求api服务
                ...
                if (Rdata.status==true){
                    if(Rdata.result.length<1){
                        ...
                        var mergedata = $.merge(data,Rdata.result);
                        viewModel.APIdata(mergedata);
        ...
        var init = function(parm){
            ...
            if(StringP.substring(StringP.length-2, StringP.length) == "CH"){
                viewModel.ismodify = 1;
                viewModel.productId = StringP.substring(0, StringP.length-2);
                $(".productName_In").attr("disabled",'disabled');
                viewModel.load();
                viewModel.getInfo();
                viewModel.APIAjax();
            ...
            }/*else if(StringP.indexOf("1")>-1 && StringP.length>1){
                viewModel.categoryId =$(".app_t1_list ul li").eq(0).attr("data-id");
                $(".app_t1_list ul li").eq(0).addClass("active");
                viewModel.appId = StringP.substring(1,StringP.length);
                viewModel.productId = "";
                viewModel.load();
                appSelect();

            }*/else{
                viewModel.productId = StringP;
               // $(".app_com_list_bo").hide();
                viewModel.load();
                viewModel.getInfo();
                //viewModel.appSelect();
            }
